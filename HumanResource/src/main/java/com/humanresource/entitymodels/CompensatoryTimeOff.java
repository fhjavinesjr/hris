 ...
 * An employee uses their accumulated COC (Compensatory Overtime Credit) hours to take a day off.
 * The employee must have sufficient COC balance before filing.
 *
 * Flow: COC Balance available → File CTO → Supervisor Approves → Day off granted, COC Balance decreases
 */
@Entity
@Table(name = "compensatory_time_off")
public class CompensatoryTimeOff implements Serializable {
    ...
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    @Column(name = "dateOfOffset", nullable = false)
    private LocalDate dateOfOffset;

    @NotNull(message = "hoursUsed is mandatory")
    @Column(name = "hoursUsed", nullable = false)
    private Double hoursUsed;

    @Column(name = "cocBalanceAtFiling")
