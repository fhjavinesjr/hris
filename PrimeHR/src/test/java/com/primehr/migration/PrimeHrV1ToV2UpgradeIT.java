...
/** Explicit real-provider upgrade gate. Ordinary test discovery excludes this IT class. */
@SpringBootTest(properties = {
        "spring.flyway.target=1",
        "spring.jpa.hibernate.ddl-auto=none"
})
class PrimeHrV1ToV2UpgradeIT {
    ...
    @Test
    void populatedV1SchemaUpgradesToV2WithoutLosingTheExistingDefinition() throws Exception {
        assertThat(initialFlyway.info().current().getVersion().getVersion()).isEqualTo("1");
        ...
        jdbcTemplate.update("INSERT INTO " + qualified + " " +
                        ...
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                id, "UPGRADE-AGENCY", "LEGACY", "Legacy definition", true, 1, null, null,
        ...
        assertThat(jdbcTemplate.queryForMap("SELECT status, definition_version, supersedes_id FROM " +
                qualified + " WHERE id = ?", id))
                .containsEntry("status", "ACTIVE")
                .containsEntry("definition_version", 1)
                .containsEntry("supersedes_id", null);
    }
}
