...
@SpringBootTest(properties = {"spring.flyway.target=14", "spring.jpa.hibernate.ddl-auto=none",
        "primehr.applicant.enabled=false"})
class PrimeHrV14ToV15UpgradeIT {
    ...
    @Test
    void populatedApplicationIntakeSurvivesScreeningPolicyMigration() throws Exception {
        assertThat(initial.info().current().getVersion().getVersion()).isEqualTo("14");
        boolean sqlServer;
        ...
        assertThat(upgraded.info().current().getVersion().getVersion()).isEqualTo("15");
        ...
        for (String table : List.of("rsp_screening_policy", "rsp_screening_policy_criterion",
                "rsp_screening_reason_code", "rsp_publication_screening_policy")) {
