CREATE TABLE "${primehrSchema}".spms_pmt (
 ...
 effective_from DATE NOT NULL, effective_to DATE, status VARCHAR(20) NOT NULL, roster_revision INTEGER NOT NULL DEFAULT 0,
 activated_by VARCHAR(100), activated_at TIMESTAMP WITH TIME ZONE,
 deactivated_by VARCHAR(100), deactivated_at TIMESTAMP WITH TIME ZONE, transition_reason VARCHAR(1000),
...
CREATE TABLE "${primehrSchema}".spms_pmt_member (
 id VARCHAR(36) PRIMARY KEY, agency_id VARCHAR(64) NOT NULL, pmt_id VARCHAR(36) NOT NULL,
 ...
 voting BOOLEAN NOT NULL, effective_from DATE NOT NULL, effective_to DATE,
 ...
 CONSTRAINT ck_spms_pmt_member_vote CHECK((member_role NOT IN ('SECRETARIAT','TECHNICAL_SUPPORT') OR voting=FALSE) AND (member_role<>'CHAIRPERSON' OR voting=TRUE))
);
