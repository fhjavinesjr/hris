...
public class ApplicantSelfServiceController {
    ...
    @PostMapping(value="/documents",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) @ResponseStatus(HttpStatus.CREATED) public ApplicantDtos.Document upload(Authentication a,@RequestParam("documentType") String type,@RequestParam(name="classification",defaultValue="SENSITIVE") String classification,@RequestPart("file") MultipartFile file){return service.upload(a.getName(),type,classification,file,null);}
    @PostMapping(value="/documents/{id}/replace",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) public ApplicantDtos.Document replace(Authentication a,@PathVariable("id") String id,@RequestParam("documentType") String type,@RequestParam(name="classification",defaultValue="SENSITIVE") String classification,@RequestPart("file") MultipartFile file){return service.upload(a.getName(),type,classification,file,id);}
    @GetMapping("/documents/{id}/content") public ResponseEntity<InputStreamResource> download(Authentication a,@PathVariable("id") String id){var d=service.download(a.getName(),id);return ResponseEntity.ok().contentType(MediaType.parseMediaType(d.mediaType())).contentLength(d.size()).header(HttpHeaders.CONTENT_DISPOSITION,ContentDisposition.attachment().filename(d.filename()).build().toString()).body(new InputStreamResource(d.stream()));}
    @DeleteMapping("/documents/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) public void deactivate(Authentication a,@PathVariable("id") String id,@RequestParam("recordVersion") long version){service.deactivateDocument(a.getName(),id,version);}
}
