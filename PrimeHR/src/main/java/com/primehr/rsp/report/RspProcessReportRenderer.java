...
public class RspProcessReportRenderer {
    ...
    private static byte[] render(String template,Map<String,Object> parameters,List<?> rows){try(InputStream in=new ClassPathResource(template).getInputStream()){JasperReport report=JasperCompileManager.compileReport(in);JasperPrint print=JasperFillManager.fillReport(report,parameters,new JRBeanCollectionDataSource(rows));return JasperExportManager.exportReportToPdf(print);}catch(Exception e){throw new IllegalStateException("Unable to generate RSP process report",e);}}
}
